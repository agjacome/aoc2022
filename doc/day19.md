...
[geodes](https://en.wikipedia.org/wiki/Geode) around the pond. Perhaps you
could use the obsidian to create some **geode-cracking robots** and break them
open?
...
after **24 minutes** by figuring out which robots to build and when to build
them.
...
blueprint's ID number** with the largest number of geodes that can be opened in
24 minutes using that blueprint. In this example, the first blueprint has ID 1
and can open 9 geodes, so its quality level is **9**. The second blueprint has
ID 2 and can open 12 geodes, so its quality level is **24**. Finally, if you
**add up the quality levels** of all of the blueprints in the list, you get
**33**.
...
Instead, for each of the first three blueprints, determine the largest number
of geodes you could open; then, multiply these three values together.
